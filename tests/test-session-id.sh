...
if [[ -f "$POST_HOOK" ]]; then
    ...
    echo "$MOCK_JSON" | CODEX_PROJECT_DIR="$TEST_DIR/project" bash "$POST_HOOK" > /dev/null 2>&1 || true
...
cd "$TEST_DIR/project"
CANCEL_OUTPUT=$(CODEX_PROJECT_DIR="$TEST_DIR/project" bash "$CANCEL_SCRIPT" 2>&1) || true
