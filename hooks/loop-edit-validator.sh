...
if [[ "$IN_PR_LOOP_DIR" == "true" ]]; then
    ...
    # For round-N-pr-resolve.md (Codex's resolution summary), validate round number
...
if is_round_file_type "$FILE_PATH_LOWER" "summary"; then
    ...
    TARGET_FILENAME=$(echo "$FILE_PATH" | sed -n 's|.*\.humanize/rlcr/[^/]*/\(.*\)$|\1|p')
    ...
    if [[ -n "$TARGET_FILENAME" ]]; then
        ...
        if [[ -n "$TARGET_ROUND" ]] && [[ "$TARGET_ROUND" != "$CURRENT_ROUND" ]] && ! is_allowlisted_file "$FILE_PATH" "$ACTIVE_LOOP_DIR"; then
            CORRECT_PATH="$ACTIVE_LOOP_DIR/round-${CURRENT_ROUND}-summary.md"
            FALLBACK="# Wrong Round Number

You tried to {{ACTION}} round-{{TARGET_ROUND}}-{{FILE_TYPE}}.md but current round is **{{CURRENT_ROUND}}**.
