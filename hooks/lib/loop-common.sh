...
_LOOP_COMMON_PROJECT_ROOT="${CODEX_PROJECT_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
...
# Build a mention string from bot names (e.g., "@reviewer @codex")
#
...
# These files are generated by the system and should not be modified manually.
...
validate_pr_resolve_round() {
    ...
    local project_root="${CODEX_PROJECT_DIR:-$(pwd)}"
    ...
    local pr_resolve_round
    pr_resolve_round=$(echo "$file_path_lower" | sed -n 's|.*round-\([0-9]*\)-pr-resolve\.md$|\1|p')

    if [[ -n "$pr_resolve_round" ]] && [[ "$pr_resolve_round" != "$pr_current_round" ]]; then
        ...
        echo "" >&2
        echo "You tried to $action_verb round-${pr_resolve_round}-pr-resolve.md but current PR loop round is **${pr_current_round}**." >&2
        echo "" >&2
